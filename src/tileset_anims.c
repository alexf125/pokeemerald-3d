...
#if WASM
static u8 *sWasmTilesetAnimDest;
static bool8 sWasmTilesetAnimSecondary;
static u16 sWasmSavedPlttUnfaded[PLTT_BUFFER_SIZE];
static u16 sWasmSavedPlttFaded[PLTT_BUFFER_SIZE];
static u32 sWasmPrimaryTilesetAnimFrame;
static u32 sWasmSecondaryTilesetAnimFrame;
#endif
...
{
#if WASM
    if (sWasmTilesetAnimDest != NULL)
    {
        const u32 globalOffset = (u8 *)dest - (u8 *)BG_VRAM;
        const u32 baseOffset = sWasmTilesetAnimSecondary ? NUM_TILES_IN_PRIMARY * TILE_SIZE_4BPP : 0;
        const u32 limit = (sWasmTilesetAnimSecondary ? NUM_TILES_TOTAL - NUM_TILES_IN_PRIMARY
                                                     : NUM_TILES_IN_PRIMARY) * TILE_SIZE_4BPP;
        if (globalOffset >= baseOffset && globalOffset + size <= baseOffset + limit)
        {
            const u8 *source = (const u8 *)src;
            u8 *target = sWasmTilesetAnimDest + globalOffset - baseOffset;
...
#if WASM
u32 WasmTilesetAnimationFrame(bool8 secondary)
{
    return secondary ? sWasmSecondaryTilesetAnimFrame : sWasmPrimaryTilesetAnimFrame;
}

void WasmApplyTilesetAnimations(const struct Tileset *tileset, u8 *dest, u16 *palettes,
                                 u16 *displayPalettes, u32 firstFrame, u32 lastFrame)
{
    const u16 savedPrimaryCounter = sPrimaryTilesetAnimCounter;
    const u16 savedPrimaryMax = sPrimaryTilesetAnimCounterMax;
    const u16 savedSecondaryCounter = sSecondaryTilesetAnimCounter;
    const u16 savedSecondaryMax = sSecondaryTilesetAnimCounterMax;
    void (*savedPrimaryCallback)(u16) = sPrimaryTilesetAnimCallback;
    void (*savedSecondaryCallback)(u16) = sSecondaryTilesetAnimCallback;
    void (*callback)(u16);
    u16 counterMax;
    ...
    {
        sWasmSavedPlttUnfaded[i] = gPlttBufferUnfaded[i];
        sWasmSavedPlttFaded[i] = gPlttBufferFaded[i];
        if (i < BG_PLTT_ID(16))
        {
            gPlttBufferUnfaded[i] = palettes[i];
            gPlttBufferFaded[i] = palettes[i];
        }
    }
    callback = NULL;
    counterMax = 0;
    ...
    {
        tileset->callback();
        callback = tileset->isSecondary ? sSecondaryTilesetAnimCallback : sPrimaryTilesetAnimCallback;
        counterMax = tileset->isSecondary ? sSecondaryTilesetAnimCounterMax : sPrimaryTilesetAnimCounterMax;
    }
    sWasmTilesetAnimDest = dest;
    sWasmTilesetAnimSecondary = tileset->isSecondary;
    if (callback != NULL && counterMax != 0 && firstFrame <= lastFrame)
    {
        for (u32 frame = firstFrame; frame <= lastFrame; frame++)
            callback(frame % counterMax);
    }
    sWasmTilesetAnimDest = NULL;
    for (u32 palette = 0; palette < 16; palette++)
    {
        // Reproduce the live display-palette pipeline for cached connected
        // tilesets, including weather color maps and an active global fade.
        ApplyWeatherColorMapToPal(palette);
        if (gPaletteFade.active && (gPaletteFade_selectedPalettes & (1 << palette)))
            BlendPalette(BG_PLTT_ID(palette), 16, gPaletteFade.y, gPaletteFade.blendColor & 0x7fff);
    }
    for (u32 i = 0; i < PLTT_BUFFER_SIZE; i++)
    {
        if (i < BG_PLTT_ID(16))
        {
            palettes[i] = gPlttBufferUnfaded[i];
            displayPalettes[i] = gPlttBufferFaded[i];
        }
        gPlttBufferUnfaded[i] = sWasmSavedPlttUnfaded[i];
        gPlttBufferFaded[i] = sWasmSavedPlttFaded[i];
    }
    sPrimaryTilesetAnimCounter = savedPrimaryCounter;
    sPrimaryTilesetAnimCounterMax = savedPrimaryMax;
    sSecondaryTilesetAnimCounter = savedSecondaryCounter;
    sSecondaryTilesetAnimCounterMax = savedSecondaryMax;
    sPrimaryTilesetAnimCallback = savedPrimaryCallback;
    sSecondaryTilesetAnimCallback = savedSecondaryCallback;
}
#endif
...
#if WASM
    // These epochs represent the absolute phase of the next callback. Some
    // secondary initializers deliberately inherit the current primary phase.
    sWasmPrimaryTilesetAnimFrame = sPrimaryTilesetAnimCounter;
    sWasmSecondaryTilesetAnimFrame = sSecondaryTilesetAnimCounter;
#endif
}
...
{
#if WASM
    sWasmPrimaryTilesetAnimFrame++;
    sWasmSecondaryTilesetAnimFrame++;
#endif
    ResetTilesetAnimBuffer();
