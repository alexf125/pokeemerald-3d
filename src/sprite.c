...
static u8 sOamCount;
#if WASM
static u8 sOamSpriteIds[128];
static s16 sOamScreenXs[128];
static s16 sOamScreenYs[128];
static s16 sSpriteScreenXs[MAX_SPRITES];
static s16 sSpriteScreenYs[MAX_SPRITES];
#endif
...
#if WASM
u32 WasmOamCount(void)
{
    return sOamCount;
}

u32 WasmOamSpriteId(u32 oamIndex)
{
    return oamIndex < sOamCount ? sOamSpriteIds[oamIndex] : MAX_SPRITES;
}

s32 WasmOamScreenX(u32 oamIndex)
{
    return oamIndex < sOamCount ? sOamScreenXs[oamIndex] : 0;
}

s32 WasmOamScreenY(u32 oamIndex)
{
    return oamIndex < sOamCount ? sOamScreenYs[oamIndex] : 0;
}
#endif
...
void AddSpritesToOamBuffer(void)
{
    ...
    {
        const u8 spriteId = sVisibleSpriteOrder[i];
        const u8 firstOam = oamIndex;
        const bool8 full = AddSpriteToOamBuffer(&gSprites[spriteId], &oamIndex);

        for (u8 entry = firstOam; entry < oamIndex; entry++)
        {
            s16 dx = (s16)gMain.oamBuffer[entry].x - gSprites[spriteId].oam.x;
            s16 dy = (s16)gMain.oamBuffer[entry].y - gSprites[spriteId].oam.y;
            if (dx >= 256) dx -= 512;
            else if (dx < -256) dx += 512;
            if (dy >= 128) dy -= 256;
            else if (dy < -128) dy += 256;
            sOamSpriteIds[entry] = spriteId;
            sOamScreenXs[entry] = sSpriteScreenXs[spriteId] + dx;
            sOamScreenYs[entry] = sSpriteScreenYs[spriteId] + dy;
        }
        if (full)
        {
            sOamCount = oamIndex;
